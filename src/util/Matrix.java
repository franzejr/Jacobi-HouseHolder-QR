...
public class Matrix {
	...
	/**
	 * Calculate the norm m[i][j] sum in module such that i != j
	 * 
	 * @param matriz
	 * @return
	 */
	public static double sumExceptDiagonal(Matrix matrix) {
		double norm = 0.0;
		for (int i = 0; i < matrix.getRowDimension(); i++) {
			for (int j = 0; j < matrix.getColumnDimension(); j++) {
				if (i != j) {
					norm += Math.abs(matrix.get(i, j));
				}
			}
		}
		return norm;
	...
	/*
	 * Returns if a matrix is upper triangular
	 * 
	 * @param matrix
	 * 
	 * @param error
	...
	public static boolean isUpperTriangular(Matrix matrix, double error) {
	...
	public static Matrix solveSubstitution(Matrix matrizTriangularInferior,
			Matrix termosIndependentes) {
		Matrix solucao = new Matrix(matrizTriangularInferior.getRowDimension(),
				1);
		for (int i = 0; i < matrizTriangularInferior.getRowDimension(); i++) {
			...
			valor = (termosIndependentes.get(i, 0) - valor)
					/ matrizTriangularInferior.get(i, i);
			solucao.set(i, 0, valor);
		}
		return solucao;
	}

	public static Matrix solveRetrosubstitution(
			Matrix matrizTriangularSuperior, Matrix termosIndependentes) {
		Matrix solucao = new Matrix(matrizTriangularSuperior.getRowDimension(),
				1);
		for (int i = matrizTriangularSuperior.getRowDimension() - 1; i >= 0; i--) {
			...
			valor = (termosIndependentes.get(i, 0) - valor)
					/ matrizTriangularSuperior.get(i, i);
			solucao.set(i, 0, valor);
		}
		return solucao;
	}
